...
import { nftExpertings } from '@/app/(site)/utils/menu';
    ...
    return (
        <div className='relative '>
            ...
            <div className='container mx-auto  font-poppins'>
                <SectionTitle title={"Our Approach to NFT Development"} des={"Embarking on Our NFT Development Excellence"} />
                <div className='flex flex-wrap justify-center lg:justify-start gap-5 mt-20  px-5 xl:px-0'>
                    {
                        nftExpertings?.map((item, index) => <ExploringCard data={item} key={index} /> )
                    }  
                </div>
            </div>
        </div>
